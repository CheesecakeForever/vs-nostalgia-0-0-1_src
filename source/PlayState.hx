...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		switch (curStage)
		{
			...
			case 'crazy-frog': //Crazy Frog's Road
				var builds:BGSprite = new BGSprite('buildings', -297, -247, 0.7, 0.7);
				add(builds);

				var road:BGSprite = new BGSprite('road', -867, -599, 1, 1);
				add(road);

			case 'duck': //The Duck Song
				var bg:BGSprite = new BGSprite('BGpaint', -1156.15, -524, 1, 1);
				add(bg);
	...
	public function endSong():Void
	{
		...
		if(achievementObj != null) {
			...
			var achieve:String = checkForAchievement(['ur_bad', 'ur_good', 'hype', 'two_keys', 'toastie', 'debugger', 'crazyfrog_nomiss']);
				...
				switch(achievementName)
				{
					case 'crazyfrog_nomiss':
						if(isStoryMode && campaignMisses + songMisses < 1 && CoolUtil.difficultyString() == 'HARD' && storyPlaylist.length <= 1 && !changedDifficulty && !usedPractice)
						{
							...
							{
								case 'crazyfrog':
									if(achievementName == 'crazyfrog_nomiss') unlock = true;
							}
						}
					case 'ur_bad':
